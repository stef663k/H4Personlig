	...
	switch (*currentState) {
		case Modtag_Adresse:
			if (recivedChar == ':') {
				...
				hexBuffer[hexIndex++] = recivedChar;
					...
					UART_PrintString("Please send a valid address terminated with ':'\n");
		...
		case Modtag_Bit_Position_Vaerdi:
			if (recivedChar == '0' || recivedChar == '1') {
					...
					PORTB = (*address & ((1 << PORTB1) | (1 << PORTB2) | (1 << PORTB3)));
		...
		default:
			UART_PrintString("Resetting to Modtag_Adresse due to unknown state\n");
			*currentState = Modtag_Adresse;
		break;
	}
}
